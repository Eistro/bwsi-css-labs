...
This is a script that implements a simple calculator. It takes two
numbers and an operation, then performs the operation and returns the
result.

The script asks the user to input the numbers and the operation to be
performed, and prints the result to the terminal window.
...
def simple_calculator(
    ...
    """
    Function that takes in two numbers and an operation (add, subtract,
    multiply, divide), then performs the operation on the two numbers and
    returns the result.
...
def request_sanitized_number(prompt: str) -> float:
    """
    function to request and santize operation user input for the
    operation

    returns:
        float: the santized numeric input by the user
    """
    while True:
        try:
